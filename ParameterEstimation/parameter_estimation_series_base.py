...
def run_multi_timeseries():
    ...
    P_meas = np.maximum(var_dict_0["P_ao"], var_dict_0["P_lv"])
    Q_lvao = var_dict_0["Q_lvao"]
    ...
    noise = 0.3
    x0 = np.array([250.0, 2.0, 1.0, 1.0, 0.32, 10.0, 0.06, 0.1, 0.003])#*(1. + noise*np.random.randn(len(active_params)))
    ...
    for kk in range(ll):
        ...
        #["V_tot", "E_max", "C_ao", "R_sys", "t_peak", "C_sv", "E_min",  "Z_ao", "R_mv"]
        x0 = x0array[kk,:]
        #x_scale = [500.0, 2.0, 2.0, 2.0, 0.32, 10.0, 0.05, 0.1, 0.005]
        #bounds = [2000.0, 5.0, 10.0, 3.0, 0.75, 30.0, 1.0, 1.0, 0.1]
        #bounds_low = [50.0, 0.9, 0.5, 0.5, 0.05, 0.5, 0.0, 0.0, 0.0]
        #x0_log = np.log(x0)
        #x_scale_log = np.abs(np.log(x_scale))
