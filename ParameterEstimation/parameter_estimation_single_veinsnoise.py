...
def run_multi_singlepoints():
    """ try many initial guesses and see how close we come """
    ...
    for meas in ["P_sys", "P_dia", "PP", "SV", "Q_max", "P_sv_sys", "P_sv_dia"]:
        measurements[meas] = ret_dict_0[meas]*(1.+ signalnoise*np.random.randn())
    ...
    for kk in range(ll):
        ...
        try:
            ...
            plt.plot(t_eval_0[np.argmin(var_dict_0["P_ao"])],measurements["P_dia"],'ko')
            plt.plot(t_eval_0[np.argmax(var_dict_0["P_ao"])],measurements["P_sys"],'ko')
            plt.plot([ t_eval_0[np.argmin(var_dict_0["P_ao"])] , 1.001*t_eval_0[np.argmin(var_dict_0["P_ao"])] ],[measurements["P_dia"], measurements["P_dia"]+measurements["PP"]],'k-')
            plt.legend()
    ...
    print(np.sum(total_list,1)/50)    
    print(np.sum(total_error,1)/50)
