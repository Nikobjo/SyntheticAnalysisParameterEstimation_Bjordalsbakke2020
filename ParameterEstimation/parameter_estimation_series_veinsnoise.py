...
def timeseries_cost_function(pars, P_meas, Q_meas, P_sv_meas, active_pars=None, ret_all=False,
    ...
    residual = np.concatenate(((var_dict["Q_lvao"] - Q_meas)/Q_scale, (P_ao - P_meas)/P_scale, (var_dict["P_sv"] - P_sv_meas)/P_scale))
    if ret_all:
...
def run_multi_timeseries():
    ...
    P_sv = var_dict_0["P_sv"]*(1. + signalnoise*np.random.randn(len(var_dict_0["P_sv"])))
    
    Q_meas=Q_lvao
    
    measurements=dict(P_meas=P_meas, Q_lvao=Q_lvao, P_sv=P_sv)
    ...
    for kk in range(ll):
        ...
        try:
            ...
            print(errorvec)
            fig = plt.figure(kk)
            plt.plot(t_eval_0, P_meas, label="data")
            plt.plot(t_eval, var_dict["P_ao"], label="opt")
            plt.legend()
            fig.savefig('AorticPressureSeriesFit'+str(kk), pad_inches = 0.2)
            plt.close()
            fig = plt.figure(kk)
            plt.plot(t_eval_0, P_sv, label="data")
            plt.plot(t_eval, var_dict["P_sv"], label="opt")
            plt.legend()
            fig.savefig('VenousPressureSeriesFit'+str(kk), pad_inches = 0.2)
            plt.close()
